...
const serverInfo = async (client, id) => {
    ...
    fs_1.default.writeFile('./data.json', JSON.stringify(data, null, 2), (err) => {
        if (err)
            throw err;
...
const userStatuses = (gMember) => {
    ...
    if (typeof db_js_1.default.data.userDB[gMember.user.id] == 'undefined') {
        db_js_1.default.data.userDB[gMember.user.id] = {
            id: gMember.user.id,
            status: status,
    ...
    const oldStatus = db_js_1.default.data.userDB[gMember.user.id].status;
    db_js_1.default.data.userDB[gMember.user.id].username = gMember.user.username;
    logger_js_1.default.info(`${logger_js_1.default.c.magenta(gMember.user.username)} changed status from ${logger_js_1.default.c.red(oldStatus)} to ${logger_js_1.default.c.red(status)}`);
    if (['online', 'idle', 'dnd', 'offline'].includes(oldStatus)) {
        db_js_1.default.data.userDB[gMember.user.id].statuses[oldStatus] +=
            now - db_js_1.default.data.userDB[gMember.user.id].last;
    }
    else {
    ...
    if (gMember.presence?.activities.length > 0) {
        ...
        games.forEach((game) => {
            if (!['PLAYING', 'LISTENING'].includes(game.type))
                return;
            if (typeof db_js_1.default.data.userDB[gMember.user.id].games[game.name] == 'undefined') {
                db_js_1.default.data.userDB[gMember.user.id].games[game.name] = 0;
            }
            db_js_1.default.data.userDB[gMember.user.id].games[game.name] +=
                now - db_js_1.default.data.userDB[gMember.user.id].last;
        });
    }
    db_js_1.default.data.userDB[gMember.user.id].statuses.total =
        db_js_1.default.data.userDB[gMember.user.id].statuses.online +
            db_js_1.default.data.userDB[gMember.user.id].statuses.idle +
            db_js_1.default.data.userDB[gMember.user.id].statuses.dnd;
    db_js_1.default.data.userDB[gMember.user.id].last = now;
    db_js_1.default.data.userDB[gMember.user.id].status = status;
...
const calcCringe = (games) => {
    ...
    if (has(games, 'League of Legends')) {
        ...
        if (time < times.hour) {
            cringe += 100;
            addText('League of Legends', 'Your have league of legends on your computer. Burn it.');
        }
            ...
            cringe += 1000;
            addText('League of Legends', 'Your a lost cause. Nothing can be done to save you.');
        }
    }
